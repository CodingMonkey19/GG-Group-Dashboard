...
export function writeConsolidation(
  ...
  const insertInvoice = db.prepare(`
    INSERT OR IGNORE INTO invoices (
      ...
      target_url, anchor_text, live_url,
      native_amount, native_currency, eur_amount, presswhizz_price_eur, savings_eur, ecb_rate, ecb_rate_as_of,
  ...
  const tx = db.transaction(() => {
    ...
    for (const row of rows) {
      const result = insertInvoice.run({
        ...
        presswhizz_price_eur: row.presswhizz_price_eur,
